...
def test_load_settings_reads_worker_limits(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	monkeypatch.setenv("PARSE_TIMEOUT_SECONDS", "12")

	settings = load_settings()

	assert settings.poll_interval_seconds == 1.5
	assert settings.lease_seconds == 45
	assert settings.parse_timeout_seconds == 12


def test_load_settings_rejects_non_positive_parse_timeout(
	monkeypatch: pytest.MonkeyPatch,
) -> None:
	monkeypatch.setenv("DATABASE_URL", "postgresql+asyncpg://worker:secret@db/skillsignal")
	monkeypatch.setenv("PARSE_TIMEOUT_SECONDS", "0")
