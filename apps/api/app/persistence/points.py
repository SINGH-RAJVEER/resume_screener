...
class PointLedger:
	...
	async def user_account(self, owner_user_id: str) -> PointAccount:
		async with self._sessions.begin() as session:
			return await ensure_user_account(session, owner_user_id)

	async def organization_account(self, organization_id: str) -> PointAccount:
		async with self._sessions.begin() as session:
			return await ensure_organization_account(session, organization_id)

	async def grant(
		self, account_id: str, amount: int, reason: str, idempotency_key: str
	) -> int:
		async with self._sessions.begin() as session:
			return await grant_in_session(
				session, account_id, amount, reason, idempotency_key
			)
	...
	async def reserve(
		...
		async with self._sessions.begin() as session:
			return await reserve_in_session(
				session, account_id, amount, purpose, idempotency_key
			)
	...
	async def settle(
		...
		async with self._sessions.begin() as session:
			await settle_in_session(session, reservation_id, charged_amount, reason)
...
async def settle_in_session(
	session: AsyncSession, reservation_id: str, charged_amount: int, reason: str
) -> None:
...
async def release_in_session(session: AsyncSession, reservation_id: str) -> None:
	reservation = await _reservation_for_update(session, reservation_id)
	if reservation.state != "reserved":
		return
...
async def ensure_user_account(session: AsyncSession, owner_user_id: str) -> PointAccount:
	return await _ensure_account(session, owner_user_id=owner_user_id)


async def ensure_organization_account(
